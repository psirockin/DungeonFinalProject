...
class Monster:
    ...
    def howtoattack(self,target):
        if self.equip == None:
            ...
            return self.attacking(self.equip, target)
...
#        print(self.name, "is dealing ", damage)
        if damage <= 0:
                damage = 1
        target.HP -= damage
        return damage

    def attacking(self, weapon, target):
        damage = 0
        ...
        if accuracy < random.randrange(100):
            ...
            return 0
        if weapon.type == "W":
            damage = self.strength + self.equip.attack - target.defense
#            pprint(self.name, "is dealing ", damage)
            if damage <= 0:
                damage = 1
            target.HP -= damage
            return damage
        elif weapon.type == "M":
            damage = self.magic + self.equip.attack - target.resist
#            print(self.name, "is dealing ", damage)
            if damage <= 0:
                damage = 1
            target.HP -= damage
